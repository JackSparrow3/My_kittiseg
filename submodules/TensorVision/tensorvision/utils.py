...
# the hypes is a dict and it store everything you want -----------------------------------------------yu
def load_modules_from_hypes(hypes, postfix=""):
    ...
    """
    modules = {}
    #the base_path towards to the folder "hypes"-------------------------------------------------------------------yu
    base_path = hypes['dirs']['base_path']
    ...
    # data_input towards to the /inputs/kitti_seg_input.py---------------------------------------------------------------yu

    f = os.path.join(base_path, hypes['model']['architecture_file'])
    arch = imp.load_source("arch_%s" % postfix, f)
    modules['arch'] = arch
    # arch towards to the /encoder/fcn8_vgg.py---------------------------------------------------------------------------yu

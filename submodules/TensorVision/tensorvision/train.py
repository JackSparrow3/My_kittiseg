...
def do_training(hypes,trainable_scopes=None,exclude_scopes=None,checkpoint_path=None):
    """
    ...
    # test on MNIST.
    #modules obtain some files like input.py architecture_file
...
#tv_graph obtain some graph like train_op loss_op that you can run directtly  ---------------------------------------------yu
        tv_graph = core.build_training_graph(hypes, queue, modules,trainable_scopes=trainable_scopes)
#tv_sess obtain some session like sess writer saver and so on
        # prepaire the tv session
        tv_sess = core.start_tv_session(hypes,exclude_scopes=exclude_scopes,checkpoint_path=checkpoint_path)

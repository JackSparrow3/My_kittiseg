...
/t/tDeep Residual Learning for Image Recognition. arXiv:1512.03385
Other variants were introduced in:
[2] Kaiming He, Xiangyu Zhang, Shaoqing Ren, Jian Sun
/t/tIdentity Mappings in Deep Residual Networks. arXiv: 1603.05027
...
units.
Typical use:
/tab from tensorflow.contrib.slim.nets import resnet_v1
...
from ResNet.resnet_utils import _get_deconv_filter as _get_deconv_filter
			...
			with slim.arg_scope([slim.batch_norm], is_training=is_training):
				...
				if include_root_block:
					if output_stride is not None:
						...
						output_stride /= 4
					net = slim.conv2d(net,64,[3,3],1,'SAME',scope='conv0')
					net = slim.conv2d(net,64,[3,3],1,'SAME',scope='conv0_0')
					net = resnet_utils.conv2d_same(net, 64, 3, stride=2, scope='conv_1')
					# TODO COMMINT
				...
				with slim.arg_scope([slim.conv2d_transpose], stride=2, activation_fn=None,
					...
					# 12x39x1024
					end_point='trans_0'
					net,end_points=_upscore_layer(net,end_points,tf.shape(end_points['resnet_v1_50/block3']),depth=1024,
					...
					net,end_points=conv(net,end_points,[3,3,2048,1024],name=end_point)

					# 24x78x1024
					net,end_points=br(net,end_points,name='BR_fuse0')
					# 24x78x1024
					end_point='trans_1'
					net, end_points = _upscore_layer(net, end_points, tf.shape(end_points['resnet_v1_50/block2']),depth=512,
						                                 wkersize=2, hkersize=2, name=end_point)
					#47x156x512
					...
					net,end_points=conv(net,end_points,[3,3,1024,512],name=end_point)
					# 47x156x512
					net,end_points=br(net,end_points,name='BR_fuse1')
					# 47x156x512
					end_point = 'trans_2'
					net, end_points = _upscore_layer(net, end_points, tf.shape(end_points['resnet_v1_50/block1']),depth=256,
					                                 wkersize=2, hkersize=2, name=end_point)
					# 94x311x256
					...
					net,end_points=conv(net,end_points,[3,3,512,256],name=end_point)
					# 94x311x256
					...
					if is_training is False:
						...
						# 188x621x64
						end_point='fuse_3'
						net=tf.concat([net,end_points['resnet_v1_50/conv_1']],axis=3,name=end_point)
						end_points[end_point]=net
					# 188x621x128
					end_point='trans_3_conv_0'
					net,end_points=conv(net,end_points,[3,3,128,16],name=end_point)
					# 188x621x16
					net,end_points=br(net,end_points,name='BR_192x624')
					# 188x621x16
					end_point='trans_4'
					net, end_points = _upscore_layer(net, end_points, tf.shape(inputs),depth=2,
					                                 wkersize=2, hkersize=2, name=end_point)
					# 375x1242x2

					# end_point='fuse_4'
					# net=tf.concat([net,inputs],axis=3,name=end_point)
					# end_points[end_point]=net
					# # 375x1242x5
					# end_point='fuse_4_conv_0'
					# net,end_points=conv(net,end_points,[3,3,5,2],name=end_point)
					# 375x1242x2

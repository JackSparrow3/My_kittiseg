        ...
        with slim.arg_scope([slim.batch_norm, slim.dropout],
            ...
            with tf.variable_scope('Upsampling'):
                with slim.arg_scope([slim.conv2d_transpose], stride=2, padding='VALID', activation_fn=None,
                    ...
                    end_point = "Conv2d_Trans_5x5"
                    net = slim.conv2d_transpose(net, 768, [4, 4], scope='Conv2d_Trans_5x5', padding='VALID')
                    end_points[end_point] = net
                    if (skip == '8s' or skip == '16s'):
                        end_point='score_mixed_6e'
                        net = end_points['Mixed_6e']
                        net = slim.conv2d(net,depth(768),[1,1],scope='score_mixed_6e')
                        end_points[end_point]=net

                        net = tf.concat([end_points['Conv2d_Trans_5x5'], end_points['score_mixed_6e']], axis=3, name='fuse_1')
                        # net = tf.add(net, end_points['Mixed_6e'], name='fuse_1')
                        end_point = "fuse_1"
                        end_points[end_point] = net
                    net = slim.conv2d_transpose(net, 288, [3, 3], scope='Conv2d_Trans_4x4')
                    end_point = "Conv2d_Trans_4x4"
                    end_points[end_point] = net
                    if skip == '8s':
                        net=end_points['Mixed_5c']
                        net = slim.conv2d(net,depth(288),[1,1],scope='score_mixed_5c')
                        end_point='score_mixed_5c'
                        end_points[end_point]=net

                        net = tf.concat([end_points['Conv2d_Trans_4x4'], end_points['score_mixed_5c']], axis=3, name='fuse_2')

...
def evaluate(hypes, sess, image_pl, inf_out):
    ...
    for phase in ['train', 'val']:
        ...
        if phase == 'val':
            ...
            print ('val for 10 times , the average is '+ str(dt))
    eval_list = []
...
def evaluate_with_crf(hypes,sess,image_pl,inf_out):
    ...
    for phase in ['train', 'val']:
        ...
        with open(data_file) as file:
            for i, datum in enumerate(file):
                    ...
                    shape = input_image.shape
                    image_to_input=np.expand_dims(input_image,axis=0)

                    feed_dict = {image_pl: image_to_input}
...
def evaluate_without_crf(hypes,sess,image_pl,inf_out):
...
def evaluate_test(hypes,sess,image_pl,inf_out):

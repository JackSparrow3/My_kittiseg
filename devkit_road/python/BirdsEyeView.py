...
class BirdsEyeView(object):
    ...
    def computeBEVLookUpTable(self, cropping_ul = None, cropping_size = None):
        ...
        shape_xzmesh1=int(self.bevParams.bev_size[0] * self.bevParams.bev_size[1])
        Z_mesh_vec = (np.reshape(XZ_mesh[1], (shape_xzmesh1), order = 'F')).astype('f4')
        shape_xzmesh0=int(self.bevParams.bev_size[0] * self.bevParams.bev_size[1])
        X_mesh_vec = (np.reshape(XZ_mesh[0], (shape_xzmesh0), order = 'F')).astype('f4')
    ...
    def transformImage2BEV(self, inImage, out_dtype = 'f4'):
        ...
        if len(inImage.shape) > 2:
            ...
            bev_int=(int(self.bevParams.bev_size[0]),int(int(self.bevParams.bev_size[1])))
            outputData = np.zeros(bev_int, dtype = out_dtype)
